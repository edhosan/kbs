<?php class Penduduk_Model extends CI_Model{
	...
	function cluster_kecamatan(){
		...
		$cluster_sql= " AND a.kecamatan IN (SELECT b.kecamatan
	...
	function cluster_kampung(){
		$user_id = $_SESSION['user'];		
		$cluster_sql= " AND dusun IN (SELECT a.id_cluster
						FROM user_wilayah a JOIN tweb_wil_clusterdesa b ON a.id_cluster = b.dusun
						WHERE a.id_user='$user_id'
						GROUP BY id_cluster)";
			return $cluster_sql;		
	...
	function list_kecamatan(){		
		$sql   = "SELECT * FROM tweb_wil_clusterdesa a WHERE a.rt = '0' AND a.rw = '0' AND a.dusun='0' ";
	...
	function list_dusun($kecamatan=""){
		$sql   = "SELECT * FROM tweb_wil_clusterdesa WHERE rt = '0' AND rw = '0' AND dusun <> '0' AND dusun <> '' AND  kecamatan = ?  ";

		if($_SESSION['grup']==4){
			$sql .= $this->cluster_kampung();	
		}

		$query = $this->db->query($sql, $kecamatan);
		$data=$query->result_array();
		return $data;
	}
	...
	function list_rw($kecamatan='', $dusun=''){
		$sql   = "SELECT * FROM tweb_wil_clusterdesa WHERE rt = '0' AND rw <> '0' AND dusun = ? AND kecamatan = ?  ";
		$query = $this->db->query($sql,array($dusun, $kecamatan));
		$data=$query->result_array();
		return $data;
	}
	...
	function list_rt($dusun='',$rw=''){
		$sql   = "SELECT * FROM tweb_wil_clusterdesa WHERE rw = ? AND dusun = ? AND rt <> '0' AND rt <> '-'";
