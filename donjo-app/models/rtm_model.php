<?php class rtm_Model extends CI_Model{
	...
	function cluster_kecamatan(){
		...
		$cluster_sql= " AND c.kecamatan IN (SELECT b.kecamatan
	...
	function list_penduduk_lepas(){
		$sql   = "SELECT p.id,p.nik,p.nama,h.nama as kk_level FROM tweb_penduduk p LEFT JOIN tweb_penduduk_hubungan h ON p.kk_level=h.id LEFT JOIN tweb_wil_clusterdesa c ON p.id_cluster = c.id WHERE (status = 1 OR status = 3) AND status_dasar = 1 AND id_rtm = 0";
