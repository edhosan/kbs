...
class User_Model extends CI_Model{
	...
	function list_user(){
		//Main Query
		$sql   = "select a.*,b.nama as grup 
				  from user a join user_grup b on a.id_grup = b.id and b.id=4 ";
	...
	public function insert_wilayah_kerja(){
		$data = $_POST;
		$user_wilayah = array(
			'id_user' => $data['userid'],
			'id_cluster' =>  $data['id_cluster']
		);
		$outp = $this->db->insert('user_wilayah',$user_wilayah);

		if($outp) $_SESSION['success']=1;
			else $_SESSION['success']=-1;	
	}

	function paging_wilayah_kerja($p=1,$o=0){

		$sql      = "SELECT COUNT(a.id) AS id 
					 FROM user a JOIN user_grup b on a.id_grup = b.id
					 JOIN user_wilayah c on a.id = c.id_user WHERE 1=1 ";
		$sql     .= $this->search_sql_wilayah_kerja();
		$query    = $this->db->query($sql);
		$row      = $query->row_array();
		$jml_data = $row['id'];
	...
	function search_sql_wilayah_kerja(){
		if(isset($_SESSION['cari'])){
				$cari = $_SESSION['cari'];
				$kw = $this->db->escape_like_str($cari);
				$kw = '%' .$kw. '%';
				$search_sql= " AND (a.username LIKE '$kw' OR a.nama LIKE '$kw' OR b.nama LIKE '$kw' OR c.id_cluster LIKE '$kw')";
				
				return $search_sql;
			}
	}

	function list_data_wilayah_kerja($o=0,$offset=0,$limit=500){

		//Ordering SQL
		switch($o){
			case 1: $order_sql = ' ORDER BY a.username'; break;
			case 2: $order_sql = ' ORDER BY a.username DESC'; break;
			case 3: $order_sql = ' ORDER BY a.nama'; break;
			case 4: $order_sql = ' ORDER BY a.nama DESC'; break;
			case 5: $order_sql = ' ORDER BY b.nama'; break;
			case 6: $order_sql = ' ORDER BY b.nama DESC'; break;
			case 7: $order_sql = ' ORDER BY c.id_cluster'; break;
			case 8: $order_sql = ' ORDER BY c.id_cluster DESC'; break;
			default:$order_sql = ' ORDER BY a.username';
		}
		...
		//Main Query
		$sql   = "SELECT c.id_user,c.id_cluster,a.username, a.nama,b.nama as grup, c.id_cluster 
				  FROM user a JOIN user_grup b ON a.id_grup = b.id
				  JOIN user_wilayah c ON a.id = c.id_user WHERE 1=1 ";
	...
	function autocomplete_wilayah_kerja(){
	...
	function delete_all_wilayah_kerja(){
		$id_cb = $_POST['id_cb'];

		if(count($id_cb)){
			foreach($id_cb as $id){
				$data = explode(":", $id);
				$sql  = "DELETE FROM user_wilayah WHERE id_user=? AND id_cluster=?";
				$outp = $this->db->query($sql, $data);
			}
		}
		else $outp = false;
	...
	function delete_wilayah_kerja($id_user='', $id_cluster=''){
		$sql  = "DELETE FROM user_wilayah WHERE id_user=? AND id_cluster=?";
		$outp = $this->db->query($sql,array($id_user, $id_cluster));
