<?php class Wilayah_Model extends CI_Model {
	...
	function cluster_kecamatan(){
		$user_id = $_SESSION['user'];		
		$cluster_sql= " AND u.kecamatan IN (SELECT b.kecamatan
						FROM user_wilayah a JOIN tweb_wil_clusterdesa b ON a.id_cluster = b.dusun
						WHERE a.id_user='$user_id'
						GROUP BY id_cluster)";
			return $cluster_sql;		
	}
	
	function cluster_kampung(){
		$user_id = $_SESSION['user'];		
		$cluster_sql= " AND u.dusun IN (SELECT a.id_cluster
						FROM user_wilayah a JOIN tweb_wil_clusterdesa b ON a.id_cluster = b.dusun
						WHERE a.id_user='$user_id'
						GROUP BY id_cluster)";
			return $cluster_sql;		
	...
	*/
	function list_data($o=0,$offset=0,$limit=500){
		...
		$sql   = "SELECT u.*,a.nama AS nama_kacamat,a.nik AS nik_kacamat,
    	(SELECT COUNT(id) FROM tweb_wil_clusterdesa WHERE rt = '0' AND rw = '0' AND dusun <> '0' AND dusun <> '' AND kecamatan = u.kecamatan ) as jumlah_kampung,
		(SELECT COUNT(rw.id) FROM tweb_wil_clusterdesa rw WHERE kecamatan = u.kecamatan AND rw <> '-' AND rt = '-') AS jumlah_rw,
		(SELECT COUNT(v.id) FROM tweb_wil_clusterdesa v WHERE kecamatan = u.kecamatan AND v.rt <> '0' AND v.rt <> '-') AS jumlah_rt,
		(SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE kecamatan = u.kecamatan) and status_dasar=1) AS jumlah_warga,
		(SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE kecamatan = u.kecamatan) AND p.sex = 1 and status_dasar=1) AS jumlah_warga_l,
		(SELECT COUNT(p.id) FROM tweb_penduduk p WHERE p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE kecamatan = u.kecamatan) AND p.sex = 2 and status_dasar=1) AS jumlah_warga_p,
		(SELECT COUNT(p.id) FROM tweb_keluarga k inner join tweb_penduduk p ON k.nik_kepala=p.id  WHERE p.id_cluster IN(SELECT id FROM tweb_wil_clusterdesa WHERE kecamatan = u.kecamatan) AND p.kk_level = 1 and status_dasar=1) AS jumlah_kk
		FROM tweb_wil_clusterdesa u LEFT JOIN tweb_penduduk a ON u.id_kepala = a.id WHERE u.rt = '0' AND u.rw = '0' AND u.dusun = '0' ";
		
		if($_SESSION['grup']==4){
			$sql .= $this->cluster_kecamatan();	
		}
	...
	function list_data_kampung($id_kecamatan=''){

		$temp = $this->cluster_by_id($id_kecamatan);
		$kecamatan = $temp['kecamatan'];

		$sql   = "SELECT u.*,a.nama AS nama_kadusun,a.nik AS nik_kadusun,
		...
		FROM tweb_wil_clusterdesa u LEFT JOIN tweb_penduduk a ON u.id_kepala = a.id WHERE u.rt = '0' AND u.rw = '0' AND u.dusun <> '0' AND u.dusun <> '' AND u.kecamatan = '$kecamatan'  ";

		if($_SESSION['grup']==4){
			$sql .= $this->cluster_kampung();	
		}
	...
	function insert_kecamatan(){
		$data = $_POST;
		$data['kecamatan']=underscore($_POST['kecamatan']);
		$outp = $this->db->insert('tweb_wil_clusterdesa',penetration($data));

		if($outp) $_SESSION['success']=1;
			else $_SESSION['success']=-1;
	}

	function insert($id_kecamatan=''){
		$data = $_POST;
		$temp = $this->cluster_by_id($id_kecamatan);
		$data['kecamatan']= underscore($temp['kecamatan']);
	...
	function delete($id=''){
        ...
        $kecamatan = (penetration($temp['kecamatan']));

        $sql = "DELETE FROM tweb_wil_clusterdesa WHERE kecamatan='$kecamatan'";
    ...
    function delete_dusun($id_kecamatan='', $id=''){
        $temp = $this->cluster_by_id($id);
...
//Bagian RT
	...
	function insert_rt($id_kecamatan='',$dusun='',$rw=''){
		$data = $_POST;
		$temp = $this->cluster_by_id($dusun);
		$data['kecamatan']=$temp['kecamatan'];
		$data['dusun']= $temp['dusun'];
	...
	function list_penduduk(){
		$sql   = "SELECT p.id,p.nik,p.nama,p.alamat_sekarang FROM tweb_penduduk p join tweb_wil_clusterdesa u on p.id_cluster = u.id WHERE p.status = 1";
	...
	function get_kecamatan(){
		$sql   = "SELECT * FROM tweb_wil_clusterdesa WHERE rt = '0' AND rw = '0' AND dusun = '0' ";
		$query = $this->db->query($sql);
		return $query->row_array();
	}
